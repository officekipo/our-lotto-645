...
function App() {
  ...
  useEffect(() => {
    ...
    fetchDraw(selectedRound).then(async (result) => {
      if (!active) return;
      if (result) { setSelectedDraw(result); setSelectedLoading(false); return; }
      const history = await fetchHistoryFromService(1000);
      if (!active) return;
      setSelectedDraw(history.find((item) => item.round === selectedRound) ?? null);
      setSelectedLoading(false);
    });
    return () => { active = false; };
  ...
  if (!latestDraw || !draw || loading || selectedLoading) {
