  ...
  return (
    <ScrollView ref={scrollRef} className={tw.screen} style={{ flex: 1, width: '100%', minWidth: 0, alignSelf: 'stretch', backgroundColor: COLORS.bg }} contentContainerClassName={tw.screenContent} contentContainerStyle={[{ flexGrow: 1, width: '100%', minWidth: 0, paddingBottom: 102 }, rnStyle(tw.screenContent)]} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ paddingHorizontal: 12 }}>
        ...
        <View className={tw.numberGridCard} style={[rnStyle(tw.numberGridCard), { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', alignContent: 'center', columnGap: 8, rowGap: 8, paddingHorizontal: 12, paddingVertical: 12 }]}>
          {Array.from({ length: 45 }, (_, index) => index + 1).map((number) => {
            ...
            return <Pressable key={number} disabled={isBlocked} onPress={() => selectionMode === 'required' ? toggleRequired(number) : toggleExcluded(number)} className={cn(tw.selectNumber, active && (selectionMode === 'required' ? tw.selectNumberRequired : tw.selectNumberExcluded), isBlocked && tw.selectNumberBlocked)} style={[{ width: 36, height: 36, minWidth: 36, maxWidth: 36, flexGrow: 0, flexShrink: 0, borderRadius: 18, borderWidth: 1, borderColor: '#E5E8EB', backgroundColor: '#FAFBFC' }, active ? { backgroundColor: selectionMode === 'required' ? COLORS.primary : COLORS.dangerSoft, borderColor: selectionMode === 'required' ? COLORS.primary : COLORS.danger, borderWidth: 2, borderRadius: 18 } : undefined]}><Text className={cn(tw.selectNumberText, active && selectionMode === 'required' && tw.selectNumberTextActive, isBlocked && tw.selectNumberTextBlocked)} style={active ? { color: selectionMode === 'required' ? '#FFFFFF' : COLORS.danger, fontWeight: '900' } : undefined}>{number}</Text></Pressable>;
          })}
        </View>
