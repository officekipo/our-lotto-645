...
function CheckScreen({ draw }: { draw: DrawData }) {
  ...
  const [result, setResult] = useState<{ matches: number[]; rank: string; bonusMatch: boolean; prize: number } | null>(null);
  ...
  async function openScanner() {
    if (!permission?.granted) {
      ...
      if (!response.granted) {
        Alert.alert('카메라 권한이 필요합니다.', 'QR 스캔을 사용하려면 카메라 권한을 허용해 주세요.');
        return;
      }
    }
    setScannerOpen(true);
  ...
  return (
    ...
    >
      ...
      <Pressable onPress={openScanner} className={cn(tw.qrButton, 'active:opacity-70 active:scale-[0.99]')} style={rnStyle(cn(tw.qrButton, 'active:opacity-70 active:scale-[0.99]'))}>
        <View className={tw.qrButtonIcon} style={rnStyle(tw.qrButtonIcon)}><Text className={tw.qrButtonIconText} style={rnStyle(tw.qrButtonIconText)}>▣</Text></View>
        <View className={tw.qrButtonCopy} style={rnStyle(tw.qrButtonCopy)}><Text className={tw.qrButtonTitle} style={rnStyle(tw.qrButtonTitle)}>QR로 번호 불러오기</Text><Text className={tw.qrButtonText} style={rnStyle(tw.qrButtonText)}>복권 QR을 스캔하면 여러 게임을 한 번에 저장합니다.</Text></View>
        <Text className={tw.qrButtonArrow} style={rnStyle(tw.qrButtonArrow)}>›</Text>
      </Pressable>
      ...
      <Modal visible={scannerOpen} animationType="slide" onRequestClose={() => setScannerOpen(false)}><View className={tw.scannerScreen} style={rnStyle(tw.scannerScreen)}><View className={tw.scannerHeader} style={rnStyle(tw.scannerHeader)}><Text className={tw.scannerTitle} style={rnStyle(tw.scannerTitle)}>로또 QR 스캔</Text><Pressable onPress={() => setScannerOpen(false)} className={tw.scannerClose} style={rnStyle(tw.scannerClose)}><Text className={tw.scannerCloseText} style={rnStyle(tw.scannerCloseText)}>닫기</Text></Pressable></View><View className={tw.scannerFrame} style={rnStyle(tw.scannerFrame)}><CameraView className={tw.scannerCamera} style={rnStyle(tw.scannerCamera)} facing="back" barcodeScannerSettings={{ barcodeTypes: ['qr'] }} onBarcodeScanned={({ data }: { data: string }) => handleQrScanned(data)} /><View pointerEvents="none" className={tw.scannerGuide} style={rnStyle(tw.scannerGuide)} /></View><Text className={tw.scannerHint} style={rnStyle(tw.scannerHint)}>복권 하단의 QR코드를 화면 안에 맞춰 주세요.</Text></View></Modal>
    </ScrollView>
  );
}
